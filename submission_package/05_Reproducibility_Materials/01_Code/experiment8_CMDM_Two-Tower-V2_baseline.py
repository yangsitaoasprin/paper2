"""
CMDM-Lab/CYP450 baseline run based on Experiment 8 logic.

This variant:
- uses 02_Data/cmdm_lab_cyp450_baseline_ready.csv
- adds focal loss and drug-side channel attention over Two-Tower V1
...
RESULT_PATH = RESULT_DIR / "cmdm_experiment8_twotowerv2_result.json"
MODEL_PATH = MODEL_DIR / "cmdm_experiment8_twotowerv2_best.pt"
...
class TwoTowerV2(nn.Module):
    def __init__(self, dropout: float = DROPOUT) -> None:
        super().__init__()
        self.drug_tower = DrugTowerV2(dropout=dropout)
        self.enzyme_tower = EnzymeTowerV2(dropout=dropout)
...
def main() -> None:
    set_seed(SEED)
    print("=" * 72)
    print("CMDM EXPERIMENT 8: Two-Tower V2 (Focal Loss + Channel Attention)")
    print("=" * 72)
    ...
    print("\n[2/4] Initializing Two-Tower V2...")
    model = TwoTowerV2(dropout=DROPOUT).to(DEVICE)
    ...
    result = {
        "experiment": "CMDM Experiment 8: Two-Tower V2 with Focal Loss and Channel Attention",
